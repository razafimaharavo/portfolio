...
export function AnimatedTextarea({
  ...
  return (
    <div className="group relative flex flex-col w-full text-left">
      ...
      <div className="relative flex items-start bg-zinc-50/5 dark:bg-zinc-950/20 backdrop-blur-[2px] rounded-t-xl overflow-hidden transition-all duration-300">
        {/* Top-Left Aligned Icon with padding */}
        <div className="absolute left-3 top-3 text-zinc-400 group-hover:text-cyan-500 group-focus-within:text-indigo-500 transition-colors duration-300">
          <Icon className="w-4 h-4" />
        </div>

        {/* Textarea element without side/top borders, only bottom */}
        <textarea
          placeholder={placeholder}
          rows={rows}
          {...(register || {})}
          {...props}
          className="w-full pl-10 pr-4 py-3 bg-transparent text-xs text-zinc-900 dark:text-white placeholder-zinc-400/75 dark:placeholder-zinc-500/75 border-b border-zinc-200 dark:border-zinc-800 focus:outline-none focus:ring-0 transition-all font-sans relative z-10 resize-y"
        />
